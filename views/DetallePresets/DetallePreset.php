...
public function save()
    {
        ...
        elseif($selectall->num_rows==1)
        {
          ...
          $query="UPDATE detalle_preset SET cantidad='".$nueva_cantidad."' WHERE id_material='".$this->id_material."'";
    ...
    {
        $query="SELECT dp.*,m.nombre as material FROM detalle_preset dp INNER JOIN materiales m ON dp.id_material = m.id_material WHERE id_detalle_preset='".$codigo."'";
        $selectall=$this->db->query($query);
       $Listdetalle_preset=$selectall->fetch_all(MYSQLI_ASSOC);
        return $Listdetalle_preset;
    }
