...
function save()
    ...
    {
        $query="SELECT op.*,deo.id_orden_producto,deo.id_detalle_preset,deo.cantidad_utilizado,m.nombre, m.largo,m.ancho,m.grueso,deo.id_detalle_orden, dp.id_material, deo.id_bodega,ma.nombre as maquina FROM detalle_orden deo INNER JOIN orden_producto op ON op.id_orden_producto=deo.id_orden_producto INNER JOIN detalle_preset dp ON dp.id_detalle_preset = deo.id_detalle_preset INNER JOIN materiales m ON m.id_material = dp.id_material INNER JOIN maquinas ma ON ma.id_maquina = op.id_maquina WHERE op.id_orden_producto ='".$codigo."'";
        $selectall=$this->db->query($query);
       $Listdetalle_orden=$selectall->fetch_all(MYSQLI_ASSOC);
        return $Listdetalle_orden;
    }
