...
class TrasladoPro extends conexion
{
    ...
    {
        $query="SELECT t.*, m.nombre FROM traslado_pro t INNER JOIN presets m ON m.id_preset=t.id_preset WHERE id_traslado_pro='".$codigo."'";
        $selectall=$this->db->query($query);
       $Listtraslado_pro=$selectall->fetch_all(MYSQLI_ASSOC);
        return $Listtraslado_pro;
    }
    ...
    {
        $query="SELECT t.*, m.nombre FROM traslado_pro t INNER JOIN presetes m ON m.id_preset=t.id_preset WHERE bodega_destino='".$codigo."' AND estado = 'No Confirmado'";
        $selectall=$this->db->query($query);
       $Listtraslado_pro=$selectall->fetch_all(MYSQLI_ASSOC);
        return $Listtraslado_pro;
    }
